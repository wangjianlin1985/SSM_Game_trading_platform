		...
		success : function (product, response, status) {
			...
			if (product) { 
				...
				$("#product_productNum_edit").validatebox({
					required : true,
					validType : "integer",
					missingMessage : "请输入游戏库存",
					invalidMessage : "游戏库存输入不对",
				});
				$("#product_price_edit").val(product.price);
				$("#product_price_edit").validatebox({
					required : true,
					validType : "number",
					missingMessage : "请输入游戏价格",
					invalidMessage : "游戏价格输入不对",
				});
